...
这个站点把 `src/content/posts/` 当成知识库入口，不是 `src/posts/`。每篇公开笔记建议保留一段 YAML frontmatter，页面会用它完成排序、关键词、主题归档和摘要展示。
...
- `category`：研究主题，数量要少而稳定，用来生成首页“研究主题”和合集页一级分组。
...
- 新文章优先写到 `src/content/posts/<主题>/<系列或文章>/index.md`。
